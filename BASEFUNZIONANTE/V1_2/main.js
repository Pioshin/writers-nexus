...
function setupEventListeners() {
    ...
    systemsList.addEventListener('click', async (e) => {
        const editButton = e.target.closest('.edit-system-btn');
        if (editButton) {
            const sysId = editButton.dataset.sysId;
            if (!sysId) return;
            const sysRef = doc(db, 'users', currentUserId, 'projects', currentProjectId, 'systems', sysId);
            const sysSnap = await getDoc(sysRef);
            if (sysSnap.exists()) {
                const sysData = sysSnap.data();
                document.getElementById('system-id').value = sysId;
                document.getElementById('sys-name').value = sysData.name || '';
                document.getElementById('sys-rules').value = sysData.rules || '';
                document.getElementById('sys-role').value = sysData.role || '';
                systemModal.classList.remove('hidden');
            }
        }
    });
...
function loadSystems(projectId) {
    const q = query(collection(db, 'users', currentUserId, 'projects', projectId, 'systems'), orderBy('name'));
    unsubscribeSystems = onSnapshot(q, (snapshot) => {
        systemsList.innerHTML = snapshot.empty ? `<p class="text-secondary p-4 text-center">Nessun sistema creato.</p>` : '';
        snapshot.forEach(doc => {
            const sys = { id: doc.id, ...doc.data() };
            const card = document.createElement('div');
            card.className = 'bg-primary p-4 rounded-lg';
            card.innerHTML = `
                <div class="flex justify-between items-center">
                    <div>
                        <p class="font-bold">${sys.name}</p>
                        <p class="text-xs text-secondary mt-1">${(sys.rules || '').substring(0, 80)}...</p>
                    </div>
                    <button class="edit-system-btn p-1 hover:accent" data-sys-id="${sys.id}"><i data-lucide="edit" class="h-4 w-4"></i></button>
                </div>
            `;
...
async function saveSystem() {
    const sysData = {
        ...
        rules: document.getElementById('sys-rules').value,
        role: document.getElementById('sys-role').value,
    };
    if (!currentProjectId || !sysData.name) {
        console.error("Nome del sistema obbligatorio.");
        return;
    }
    
    const sysId = document.getElementById('system-id').value;
    const collectionRef = collection(db, 'users', currentUserId, 'projects', currentProjectId, 'systems');
    
    try {
        if (sysId) {
            await setDoc(doc(collectionRef, sysId), sysData, { merge: true });
        } else {
            await addDoc(collectionRef, {...sysData, createdAt: Timestamp.now() });
        }
    } catch(error) {
        console.error("Errore nel salvataggio del sistema: ", error);
    }
