...
async function fetchSuggestions() {
  ...
  if (!projectId) {
    log('[ERRORE] Nessun progetto attivo');
    return;
  }
  fetchBtn.disabled = true;
  fetchBtn.textContent = 'Richiedo...';
  ...
  try {
    ...
    if (!AIService || !AIService.complete) {
      log('[ERRORE] AIService non disponibile (mancato import).');
      return;
    }
    const cfg = AIService.getConfig ? AIService.getConfig() : {};
    if (!cfg.model || (!cfg.baseUrl && cfg.provider !== 'google')) {
      log(
        '[ERRORE] Configurazione AI incompleta: imposta provider, baseUrl (se richiesto) e model nelle impostazioni.'
      );
      return;
    }
    ...
    try {
      ...
      log('[ERRORE] Parsing JSON fallito');
      return;
    }
    if (!Array.isArray(arr)) {
      log('[ERRORE] Formato risposta non array');
      return;
    }
    ...
    suggestionsState = Array.from(map.values()).filter(s =>
      scenes.find(c => c.id === s.id)
    );
    if (suggestionsState.length < rawSuggestionsCount) {
      log(
        `Deduplicate: ${rawSuggestionsCount - suggestionsState.length} suggerimenti duplicati scartati.`
      );
    }
    selection.clear();
    ...
    updateSummary();
    log(
      `Ricevuti ${rawSuggestionsCount} suggerimenti (unici: ${suggestionsState.length}).`
    );
  } catch (e) {
    console.error(e);
    log('[ERRORE] Richiesta fallita');
  } finally {
    fetchBtn.disabled = false;
    fetchBtn.textContent = 'Richiedi Suggerimenti';
  }
}
...
function renderBody() {
  ...
  const filtered = suggestionsState
  ...
  tbodyEl.querySelectorAll('input[data-sel]').forEach(chk => {
    chk.addEventListener('change', () => {
      ...
      if (chk.checked) selection.add(id);
...
function updateButtons() {
  ...
  if (autoBtn)
    autoBtn.disabled = !suggestionsState.some(
      s => !s.applied && s.confidence >= thr
    );
  if (clearBtn) clearBtn.disabled = suggestionsState.length === 0;
  if (selectAllEl)
    selectAllEl.checked =
      selection.size && selection.size === suggestionsState.length;
}
...
function updateSummary() {
  ...
  if (!suggestionsState.length) {
    summaryEl.textContent = '(nessun suggerimento)';
    return;
  }
...
function getSceneTitle(id) {
  return sceneCache?.find(s => s.id === id)?.title || id;
}
function getSceneExcerpt(id) {
  return (sceneCache?.find(s => s.id === id)?.content || '')
    .slice(0, 160)
    .replace(/\s+/g, ' ');
}
function getCurrentStageOf(id) {
  return sceneCache?.find(s => s.id === id)?.stageKey;
}

async function applySelected() {
  await ensureSceneCache();
  applyBtn.disabled = true;
  applyBtn.textContent = 'Applico...';
  try {
    for (const s of suggestionsState) {
      if (!selection.has(s.id) || s.applied) continue;
      const sc = sceneCache.find(x => x.id === s.id);
      if (!sc) continue;
...
async function autoApplyThreshold() {
  ...
  autoBtn.textContent = 'Auto-applico...';
  try {
    for (const s of suggestionsState) {
