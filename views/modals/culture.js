...
function init() {
  ...
  // Tabs
  tabDetailsBtn = document.getElementById('cult-tab-details');
  tabAiBtn = document.getElementById('cult-tab-ai');
  tabDetailsPane = document.getElementById('cult-tab-details-pane');
  tabAiPane = document.getElementById('cult-tab-ai-pane');
  tabDetailsBtn?.addEventListener('click', () => switchTab('details'));
  tabAiBtn?.addEventListener('click', () => switchTab('ai'));

  // IA controls
  aiInstr = document.getElementById('ai-cult-instructions');
  aiTemp = document.getElementById('ai-cult-temp');
  aiTempVal = document.getElementById('ai-cult-temp-val');
  aiMaxTok = document.getElementById('ai-cult-maxtok');
  aiUseCtx = document.getElementById('ai-cult-use-context');
  aiCtxStatus = document.getElementById('ai-cult-context-status');
  aiGenre = document.getElementById('ai-cult-genre');
  aiTone = document.getElementById('ai-cult-tone');
  aiGenBtn = document.getElementById('ai-cult-generate');
  aiImpBtn = document.getElementById('ai-cult-improve');
  aiStopBtn = document.getElementById('ai-cult-stop');
  aiOut = document.getElementById('ai-cult-output');
  aiApply = document.getElementById('ai-cult-apply');
  aiCopy = document.getElementById('ai-cult-copy');
  aiTemp?.addEventListener(
    'input',
  ...
  aiCopy?.addEventListener('click', () =>
    navigator.clipboard?.writeText(aiOut.textContent || '')
  );
...
function buildUserPrompt(mode) {
  ...
  const base =
    mode === 'create'
      ? 'Crea un elemento culturale completo (ruoli, valori, rituali, conflitti interni).'
      : 'Migliora/Completa questo elemento mantenendo coerenza.';
  return [
    base,
    aiInstr?.value?.trim() ? `Istruzioni: ${aiInstr.value.trim()}` : null,
    ctx.genre || ctx.tone
      ? `Genere/Tono: ${[ctx.genre, ctx.tone].filter(Boolean).join(' | ')}`
      : null,
    ctx.name || ctx.details ? `Dati attuali: ${JSON.stringify(ctx)}` : null,
  ]
    .filter(Boolean)
    .join('\n\n');
}
...
async function buildContext() {
  ...
  const pid = await DataManager.getCurrentProjectId();
  if (!pid) {
    aiCtxStatus && (aiCtxStatus.textContent = 'Nessun progetto');
    return '';
  }
  ...
  const center = txt => {
    ...
    if (idx === -1) return txt.slice(0, 1000);
    const w = 500;
    const st = Math.max(0, idx - w);
    const en = Math.min(txt.length, idx + w);
    return (
      (st > 0 ? '...' : '') + txt.slice(st, en) + (en < txt.length ? '...' : '')
    );
  };
  ...
  if (sel.length < 3 && scenes.length) {
    const byOrder = [...scenes].sort((a, b) => (a.order || 0) - (b.order || 0));
    const seed = sel[0]?.s || byOrder[0];
    const idx = byOrder.findIndex(x => x.id === seed.id);
    const neigh = [idx - 1, idx, idx + 1, idx + 2].filter(
      i => i >= 0 && i < byOrder.length
    );
    const ids = new Set(sel.map(x => x.s.id));
    for (const i of neigh) {
      const sc = byOrder[i];
      if (!ids.has(sc.id)) {
        const t = [sc.title, sc.synopsis, sc.content]
          .filter(Boolean)
          .join('\n');
        sel.push({ s: sc, t, sc: scoreText(t) });
        ids.add(sc.id);
      }
      if (sel.length >= 5) break;
    }
  }
  const sceneLines = sel.map(
    x => `- ${x.s.title ? '[' + x.s.title + '] ' : ''}${center(x.t)}`
  );
  const ideaHits = [];
  for (const it of ideas) {
    ...
    if (sc > 0) {
      ideaHits.push(`- ${center(c).slice(0, 700)}`);
      if (ideaHits.length >= 4) break;
    }
  }
  const plLines = (plotlines || [])
    .slice(0, 5)
    .map(
      pl =>
        `- ${pl.name}${pl.description ? ': ' + String(pl.description).slice(0, 120) : ''}`
    );
  aiCtxStatus &&
    (aiCtxStatus.textContent = `Contesto: ${sceneLines.length} scene, ${ideaHits.length} idee`);
  return [
    plLines.length ? 'Linee narrative:' : null,
    ...plLines,
    sceneLines.length ? 'Scene correlate:' : null,
    ...sceneLines,
    ideaHits.length ? 'Idee correlate:' : null,
    ...ideaHits,
  ]
    .filter(Boolean)
    .join('\n');
...
function applyAI() {
  ...
  try {
    ...
    toast.error('JSON IA non valido');
    return;
  }
  if (obj.name) nameInput.value = obj.name;
  if (obj.details) detailsInput.value = obj.details;
}
