...
function buildUserPrompt(mode){
	...
	const base = mode === 'create' ? 'Crea un oggetto narrativo coerente e iconico.' : 'Migliora/Completa questo oggetto senza perdere coerenza.';
	return [
		base,
		...
		ctx.name || ctx.description || ctx.importance ? `Dati attuali: ${JSON.stringify(ctx)}` : null
	].filter(Boolean).join('\n\n');
}
