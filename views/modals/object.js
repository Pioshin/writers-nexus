...
function init() {
  ...
  // Tabs
  tabDetailsBtn = document.getElementById('obj-tab-details');
  tabAiBtn = document.getElementById('obj-tab-ai');
  tabDetailsPane = document.getElementById('obj-tab-details-pane');
  tabAiPane = document.getElementById('obj-tab-ai-pane');
  tabDetailsBtn?.addEventListener('click', () => switchTab('details'));
  tabAiBtn?.addEventListener('click', () => switchTab('ai'));

  // IA controls
  aiInstr = document.getElementById('ai-obj-instructions');
  aiTemp = document.getElementById('ai-obj-temp');
  aiTempVal = document.getElementById('ai-obj-temp-val');
  aiMaxTok = document.getElementById('ai-obj-maxtok');
  aiUseCtx = document.getElementById('ai-obj-use-context');
  aiCtxStatus = document.getElementById('ai-obj-context-status');
  aiGenre = document.getElementById('ai-obj-genre');
  aiTone = document.getElementById('ai-obj-tone');
  aiGenBtn = document.getElementById('ai-obj-generate');
  aiImpBtn = document.getElementById('ai-obj-improve');
  aiStopBtn = document.getElementById('ai-obj-stop');
  aiOut = document.getElementById('ai-obj-output');
  aiApply = document.getElementById('ai-obj-apply');
  aiCopy = document.getElementById('ai-obj-copy');
...
function buildUserPrompt(mode) {
  ...
  const base =
    mode === 'create'
      ? 'Crea un oggetto narrativo coerente e iconico.'
      : 'Migliora/Completa questo oggetto senza perdere coerenza.';
  return [
    base,
    ...
    ctx.genre || ctx.tone
      ...
      : null,
    ctx.name || ctx.description || ctx.importance
      ? `Dati attuali: ${JSON.stringify(ctx)}`
      : null,
  ]
    .filter(Boolean)
    .join('\n\n');
...
function applyAI() {
  ...
  if (obj.importance) importanceInput.value = obj.importance;
}
