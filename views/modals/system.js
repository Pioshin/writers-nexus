...
function buildUserPrompt(mode){
	...
	const base = mode === 'create' ? 'Crea un sistema coerente con regole, costi e limiti.' : 'Migliora/Completa questo sistema mantenendo coerenza.';
	return [
		base,
		...
		ctx.name || ctx.rules || ctx.role ? `Dati attuali: ${JSON.stringify(ctx)}` : null
	].filter(Boolean).join('\n\n');
}
