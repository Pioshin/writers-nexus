...
const RELATIONSHIP_TYPES = {
  ...
  Amore: 'Amore',
  Famiglia: 'Famiglia',
  'Mentore/Allievo': 'Mentore/Allievo',
  Rivalità: 'Rivalità',
  Inimicizia: 'Inimicizia',
  Rispetto: 'Rispetto',
  Sospetto: 'Sospetto',
};

let modal, form, modalTitle, saveBtn, cancelBtn;
let characterIdInput,
  nameInput,
  roleInput,
  appearanceInput,
  psychologyInput,
...
function init() {
  ...
  if (aiTempRange && aiTempVal) {
    aiTempRange.addEventListener(
      'input',
      () => (aiTempVal.textContent = aiTempRange.value)
    );
  }
  // IA buttons
...
function populateSelects() {
  NARRATIVE_TAGS.archetype.forEach(tag => {
    archetypeSelect.add(
      new Option(tag.charAt(0).toUpperCase() + tag.slice(1), tag)
    );
  });
  NARRATIVE_TAGS.role.forEach(tag => {
    narrativeRoleSelect.add(
      new Option(tag.charAt(0).toUpperCase() + tag.slice(1), tag)
    );
  });
  NARRATIVE_TAGS.importance.forEach(tag => {
    importanceSelect.add(
      new Option(tag.charAt(0).toUpperCase() + tag.slice(1), tag)
    );
  });
}
...
async function open(character = {}) {
  ...
  if (!currentProjectId) {
    ...
    toast.error(
      'Nessun progetto selezionato. Seleziona un progetto dalla dashboard per aggiungere personaggi.'
    );
    return;
  }

  try {
    allCharacters = await DataManager.getProjectItems(
      currentProjectId,
      'characters'
    );
  } catch (error) {
  ...
  try {
    ...
    if (project) {
      if (aiGenreInput && !aiGenreInput.value)
        aiGenreInput.value = project.genre || '';
      if (aiToneInput && !aiToneInput.value)
        aiToneInput.value = project.tone || '';
    }
  } catch {}
}
...
async function save() {
  const relationships = [];
  document
    .querySelectorAll('#character-relations-editor .relation-row')
    .forEach(row => {
  ...
  try {
    await DataManager.saveProjectItem(
      currentProjectId,
      'characters',
      characterData
    );
    ...
    toast.error(
      `Errore durante il salvataggio del personaggio: ${error.message}`
    );
  }
}
...
function addRelationRow(relation = {}) {
  ...
  targetSelect.className =
    'relation-target flex-1 px-4 py-2 bg-primary border border-accent/30 rounded-lg';
  ...
  typeSelect.className =
    'relation-type flex-1 px-4 py-2 bg-primary border border-accent/30 rounded-lg';
...
function buildUserPrompt(mode) {
  ...
  const base =
    mode === 'create'
      ? 'Crea un personaggio coerente con le istruzioni.'
      : 'Migliora/Completa questo personaggio mantenendo i campi già buoni e armonizzando il resto.';
  return [
    base,
    instructions ? `Istruzioni: ${instructions}` : null,
    ctx.genre || ctx.tone
      ? `Genere/Tono del romanzo: ${[ctx.genre, ctx.tone].filter(Boolean).join(' | ')}`
      : null,
    ctx.name ||
    ctx.role ||
    ctx.appearance ||
  ...
  if (Array.isArray(obj.relationships)) {
    ...
    renderRelations(
      obj.relationships.map(r => ({
        targetCharacterId: map.get((r.targetName || '').toLowerCase()) || '',
        type: r.type || 'Amicizia',
      }))
    );
  }
}
...
async function buildManuscriptContext() {
  ...
  const plotlines =
    (await DataManager.getProjectItems(projectId, 'plotlines')) || [];
  const targetName = (nameInput?.value || '').trim();
  const esc = s => s.replace(/[.*+?^${}()|[\]\\]/g, '\\$&');
  const tokens = targetName
    ? targetName.split(/\s+|-/).filter(t => t.length >= 3)
    : [];
  const fullRe = targetName
    ? new RegExp(`\\b${esc(targetName)}\\b`, 'i')
    : null;
  ...
  scored.sort(
    (a, b) => b.score - a.score || (a.s.order || 0) - (b.s.order || 0)
  );
  ...
  if (selected.length < 3 && scenes.length) {
    ...
    const neighborIdxs = [idx - 1, idx, idx + 1, idx + 2, idx - 2].filter(
      i => i >= 0 && i < byOrder.length
    );
    for (const i of neighborIdxs) {
      const sc = byOrder[i];
      if (!pickedIds.has(sc.id)) {
        const text = [sc.title, sc.synopsis, sc.content]
          .filter(Boolean)
          .join('\n');
    ...
    for (const it of ideas) {
      ...
      if (sc > 0)
        ideaHits.push({
          it,
          sc,
          snippet: centerSnippet(content).slice(0, 800),
        });
  ...
  const relNames = otherChars
    ...
    .slice(0, 15)
    .map(c => c.name);
  const sceneLines = selected.map(
    x => `- ${x.s.title ? '[' + x.s.title + ']' : ''} ${centerSnippet(x.text)}`
  );
  ...
  const plLines = plotlines
    ...
    .map(
      pl =>
        `- ${pl.name}${pl.description ? ': ' + String(pl.description).slice(0, 140) : ''}`
    );
  const ctx = [
    ...
    .join('\n');
  aiContextStatus &&
    (aiContextStatus.textContent = sceneLines.length
      ? `Contesto: ${sceneLines.length} scene, ${ideaLines.length} idee`
      : 'Contesto: nessuna scena trovata');
  return ctx;
}
