...
function setRelationFilter(sourceId, category) {
  if (
    ...
    activeRelationFilter.sourceId === sourceId &&
    activeRelationFilter.category === category
  ) {
...
async function loadIdeas() {
  ...
  ideas.forEach((idea, index) => {
    ...
    ideaEl.className =
      'bg-primary p-3 rounded-lg border border-accent/30 text-sm text-secondary relative group';
    ideaEl.innerHTML = `
    ...
    overlayModule.addOverlayTo(ideaEl, {
      ...
      onDelete: async () => {
        if (
          await window.appConfirm('Eliminare questa idea?', {
...
async function loadCharacters() {
  ...
  const allCharacters = await DataManager.getProjectItems(
    currentProjectId,
    'characters'
  );
    ...
    if (filterValue) {
      filteredCharacters = filteredCharacters.filter(char => {
        ...
        const valueToCompare =
          category === 'role' ? char.narrativeRole : charProperty;

        return (
          valueToCompare &&
          valueToCompare.toLowerCase() === filterValue.toLowerCase()
        );
      });
    }
  }
  ...
  if (activeRelationFilter) {
    ...
    if (sourceChar && sourceChar.relationships) {
      highlightedIds = sourceChar.relationships
        .filter(
          rel => getRelationCategory(rel.type) === activeRelationFilter.category
        )
  ...
  filteredCharacters.forEach(character => {
    ...
    const tagsHTML = [
      ...
      .map(
        tag =>
          `<span class="bg-primary text-secondary text-xs font-medium me-2 px-2.5 py-0.5 rounded-full">${tag}</span>`
      )
      .join('');
    ...
    const relationsHTML = (character.relationships || [])
      .map(rel => {
        ...
        const initials = targetChar.name
          .split(' ')
          .map(n => n[0])
          .join('');
    ...
    card.addEventListener('click', e => {
      if (
        ...
        !e.target.closest('.relation-avatar')
      ) {
...
async function loadSwordSample(url = 'assets/sword.mp3') {
  ...
  try {
    ...
    console.warn('Impossibile caricare sample spade:', e);
  }
}
...
async function loadHeartSample(url = 'assets/battito.mp3') {
  ...
  try {
    ...
    console.warn('Impossibile caricare sample battito:', e);
  }
}
...
async function loadLocations() {
  ...
  if (!listEl) return;
  const items = await DataManager.getProjectItems(
    currentProjectId,
    'locations'
  );
  listEl.innerHTML = '';
  ...
  items.forEach(loc => {
    ...
    el.className =
      'bg-primary p-3 rounded-lg text-sm text-secondary relative group';
    el.innerHTML = `
    ...
    overlayModule.addOverlayTo(el, {
      ...
      onDelete: async () => {
        if (
          await window.appConfirm(`Eliminare il luogo \"${loc.name}\"?`, {
            title: 'Conferma eliminazione',
            confirmText: 'Elimina',
          })
        ) {
...
async function loadObjects() {
  ...
  items.forEach(obj => {
    ...
    overlayModule.addOverlayTo(el, {
      ...
      onDelete: async () => {
        if (
          await window.appConfirm(`Eliminare l'oggetto \"${obj.name}\"?`, {
            title: 'Conferma eliminazione',
            confirmText: 'Elimina',
          })
        ) {
...
async function loadSystems() {
  ...
  items.forEach(sys => {
    ...
    overlayModule.addOverlayTo(el, {
      ...
      onDelete: async () => {
        if (
          await window.appConfirm(`Eliminare il sistema \"${sys.name}\"?`, {
            title: 'Conferma eliminazione',
            confirmText: 'Elimina',
          })
        ) {
...
export default {
  init: async function (dataManager, firebaseSync, modalLoader, viewSwitcher) {
    ...
    if (!currentProjectId) {
      document.getElementById('view-ideation').innerHTML =
    ...
    document
      .getElementById('new-idea-btn')
      .addEventListener('click', handleNewIdeaClick);
    document
      .getElementById('new-location-btn')
      .addEventListener('click', () => locationModal.open());
    document
      .getElementById('new-object-btn')
      .addEventListener('click', () => objectModal.open());
    document
      .getElementById('new-system-btn')
      .addEventListener('click', () => systemModal.open());
    document
      .getElementById('new-geography-btn')
      .addEventListener('click', handleNewGeographyClick); // New button listener
    document
      .getElementById('new-history-btn')
      .addEventListener('click', handleNewHistoryClick); // New button listener
    document
      .getElementById('new-culture-btn')
      .addEventListener('click', handleNewCultureClick); // New button listener
    ...
    try {
      ...
      aiBtn?.addEventListener('click', async () => {
        const project = currentProjectId
          ? await DataManager.getProject(currentProjectId)
          : null;
      ...
      console.warn('AI modal non disponibile:', e);
    }
