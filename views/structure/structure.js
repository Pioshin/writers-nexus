...
async function init(dataManager, firebaseSync, modalLoader, viewSwitcher) {
    ...
    document.getElementById('open-hero-suggestions')?.addEventListener('click', () => {
...
async function loadUnassignedScenes(container) {
    if (!container) return;
    container.innerHTML = '';
    const scenes = await DataManager.getScenesForStage(currentProjectId, UNASSIGNED_KEY);
    if (!scenes.length) {
        container.innerHTML = '<p class="text-xs text-secondary italic col-span-full">Nessuna scena non assegnata.</p>';
        return;
    }
    scenes.forEach(scene => container.appendChild(createSceneCard(scene, true)));
    // reinizializza selezioni non più presenti
    unassignedSelection.forEach(id => { if (!scenes.find(s => s.id === id)) unassignedSelection.delete(id); });
    updateUnassignedSelectionUI();
}

function bindUnassignedControls() {
    document.getElementById('unassigned-refresh')?.addEventListener('click', async () => {
    ...
    selectAll?.addEventListener('change', () => {
        ...
        if (selectAll.checked) cards.forEach(c => unassignedSelection.add(c.dataset.sceneId));
        updateUnassignedSelectionUI();
    });
    ...
    batchApply?.addEventListener('click', async () => {
        ...
        batchApply.disabled = true; batchApply.textContent = 'Assegno...';
        try {
            for (const id of ids) {
        ...
        const stageContainer = document.querySelector(`[data-stage-key="${target}"]`);
        if (stageContainer) loadScenesForStage(target, stageContainer.querySelector('.scenes-list'));
    });
}
...
async function loadScenesForStage(stageKey, scenesListContainer) {
    ...
    let dirty = false;
    let maxOrder = scenes.reduce((m,s) => typeof s.order === 'number' ? Math.max(m,s.order) : m, 0);
    if (!maxOrder) maxOrder = 0;
    scenes.forEach((s,i) => {
        if (typeof s.order !== 'number') { s.order = (maxOrder + (i+1)*100); dirty = true; }
    });
    if (dirty) {
...
function createSceneCard(scene, isUnassigned = false) {
    ...
    badge.className = 'absolute top-1 left-1 stage-badge-mini ' + stageBadgeClass(scene.stageKey);
    ...
    if (select) {
        ...
        select.addEventListener('change', async (e) => {
            ...
            const stageContainer = document.querySelector(`[data-stage-key="${newKey}"]`);
            if (stageContainer) {
            ...
            if (scene.stageKey && scene.stageKey !== 'unassigned') {
                const oldContainer = document.querySelector(`[data-stage-key="${scene.stageKey}"]`);
                if (oldContainer) loadScenesForStage(scene.stageKey, oldContainer.querySelector('.scenes-list'));
            }
        });
    }
    ...
    cardEl.querySelector('[data-edit-scene]')?.addEventListener('click', async (e) => {
        e.stopPropagation();
    ...
    if (isUnassigned) {
        ...
        chk.addEventListener('change', () => {
            if (chk.checked) unassignedSelection.add(scene.id); else unassignedSelection.delete(scene.id);
            updateUnassignedSelectionUI();
        });
    ...
    listEl.addEventListener('drop', async (e) => {
        ...
        if (oldStage && oldStage !== stageKey) {
            if (oldStage === UNASSIGNED_KEY) {
                ...
                const oldContainer = document.querySelector(`[data-stage-key="${oldStage}"]`);
                if (oldContainer) {
...
function shortStageLabel(key) {
    switch(key) {
        case 'ordinary_world': return 'ORD';
        case 'call_to_adventure': return 'CALL';
        case 'refusal_of_call': return 'RIF';
        case 'meeting_mentor': return 'MENT';
        case 'crossing_threshold': return 'SOGL';
        case 'tests_allies_enemies': return 'PROVE';
        case 'inmost_cave': return 'CAVER';
        case 'ordeal': return 'PROVA';
        case 'reward': return 'RIC';
        case 'road_back': return 'RITOR';
        case 'resurrection': return 'RES';
        case 'return_with_elixir': return 'ELIS';
...
function fullStageLabel(key) {
    switch(key) {
        case 'ordinary_world': return 'Mondo Ordinario';
        case 'call_to_adventure': return 'Chiamata all\'Avventura';
        case 'refusal_of_call': return 'Rifiuto della Chiamata';
        case 'meeting_mentor': return 'Incontro con il Mentore';
        case 'crossing_threshold': return 'Superamento della Prima Soglia';
        case 'tests_allies_enemies': return 'Prove, Alleati e Nemici';
        case 'inmost_cave': return 'Avvicinamento alla Caverna';
        case 'ordeal': return 'Prova Centrale';
        case 'reward': return 'Ricompensa';
        case 'road_back': return 'La Via del Ritorno';
        case 'resurrection': return 'Resurrezione';
        case 'return_with_elixir': return 'Ritorno con l\'Elisir';
        case 'unassigned': return 'Non Assegnata';
        default: return key || '';
    }
}

function purgeSceneDuplicates(sceneId, keepStageKey) {
    const cards = document.querySelectorAll(`.scene-card[data-scene-id="${sceneId}"]`);
    if (cards.length <= 1) return;
    cards.forEach(card => {
        const stageContainer = card.closest('[data-stage-key]');
        const stage = stageContainer ? stageContainer.getAttribute('data-stage-key') : (card.closest('#unassigned-scenes') ? UNASSIGNED_KEY : null);
...
function enableCardReorder(cardEl, scene, stageKey, listEl) {
    ...
    cardEl.addEventListener('drop', async (e) => {
        ...
        if (siblings.some(s => s.order === newOrder)) {
            ...
            const compactSiblings = siblings.map(s=>s.id===scene.id?{...s,order:compactTargetOrder}:s);
            if (before) newOrder = compactTargetOrder - 50; else newOrder = compactTargetOrder + 50;
        }
...
function snapshotStage(stageKey, listEl) {
    return Array.from(listEl.querySelectorAll('.scene-card')).map(c => ({ id: c.dataset.sceneId, order: Number(c.dataset.order), stageKey }));
}
...
function updateUnassignedSelectionUI() {
    ...
    const batchTarget = document.getElementById('unassigned-batch-target');
    const selectAll = document.getElementById('unassigned-select-all');
    const totalCards = document.querySelectorAll('#unassigned-scenes .scene-card').length;
    if (counter) counter.textContent = `${unassignedSelection.size} selezionate`;
    if (batchApply) batchApply.disabled = !(unassignedSelection.size && batchTarget?.value);
    if (selectAll) selectAll.checked = (unassignedSelection.size && unassignedSelection.size === totalCards);
}
